...
export function createFileUploadExtension() {
    return {
        ...
        async onChangeFileInput() {
            ...
            const [maxSizeMB, forbiddenExtensionsStr] = await Promise.all([
                this.orm.call("ir.config_parameter", "get_param", [
                    "dms.binary_max_size",
                    "25",
                ]),
                this.orm.call("ir.config_parameter", "get_param", [
                    "dms.forbidden_extensions",
                    "",
                ]),
            ]);

            const maxSize = parseInt(maxSizeMB) * 1024 * 1024;
            const forbiddenExtensions = forbiddenExtensionsStr
                ? forbiddenExtensionsStr.split(",").map((e) => e.trim().toLowerCase())
                : [];
            ...
            for (const file of files) {
                ...
                if (maxSize > 0 && file.size > maxSize) {
                    invalidFiles.push({
                        name: file.name,
                        reason: _t("File is too large (max %s MB)", maxSizeMB),
                    });
                } else if (forbiddenExtensions.includes(extension)) {
                    invalidFiles.push({
                        name: file.name,
                        reason: _t("File extension is not allowed"),
                    });
                } else {
            ...
            if (invalidFiles.length > 0) {
                invalidFiles.forEach((f) => {
                    this.notification.add(`${f.name}: ${f.reason}`, {
                        type: "danger",
                    });
                });
            }
